...
def create_transaction(session: Session, *, user_id: int, amount: float, kind: Kind, occurred_at: date, description: Optional[str], category_id: Optional[int]) -> Transaction:
    ...
    return TransactionRepo(session).create(
        user_id=user_id, amount=amount, kind=kind, occurred_at=occurred_at, description=description, category_id=category_id
    )

def list_transactions(session: Session, *, user_id: int, kind: Optional[Kind] = None, category_id: Optional[int] = None):
...
def list_in_range(session: Session, *, user_id: int, start: Optional[date], end: Optional[date]):
